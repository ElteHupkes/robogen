...
//	roboRot.makeRotate(osg::inDegrees(startingAzimuth), osg::Vec3(0,0,1));
	...
	for (unsigned int i = 0; i < robots.size(); ++i) {
		...
		robot->translateRobot(osg::Vec3(x, y,
				robogenConfig_->getTerrainConfig()->getHeight() + inMm(2) - minZ));

//		std::cout << "Placing robot at " << x << ", " << y << std::endl;
		x += spacing;
		if (i > 0 && (i % rowSize) == 0) {
			y += spacing;
			x = 0;
